...
export class CommentService {
  ...
  async findCommentByRootCommentId(request: CommentRequest): Promise<CommentVo[]> {
    return this.entityManager.createQueryBuilder().from(CommentEntity, "comment")
      ...
      .getManyAndCount().then((result) => {
        ...
        userIdList.push(...result.flatMap((commentVo) => {
  ...
  async findCommentByPostId(request: CommentRequest): Promise<CommentVo[]> {
    return this.entityManager.createQueryBuilder()
      ...
      .where({
        postId: request.postId, postType: request.postType, rootCommentId: null, validStatus: CommonValidStatus.VALID
      })
      .skip((request.page.pageIndex - 1) * request.page.pageSize)
      .take(request.page.pageSize)
      .getManyAndCount()
      .then((result) => {
        ...
        for (let comment of commentList) {
          ...
          commentVo.commentatorId = comment.commentatorId;
          commentVo.page = Page.getPage(request.page, commentCount);
          this.entityManager.createQueryBuilder().from(CommentEntity, "comment")
            .addOrderBy("comment.createTime", "ASC")
            .where({
              postId: request.postId,
              postType: request.postType,
        ...
        result.flatMap((commentVo) => {
          ...
          userIdList.push(...commentVo.childrenComment.flatMap((childCommentVo) => {
            return [childCommentVo.commentatorId, childCommentVo.parentCommentatorId];
          }));
          this.usersService.findOneByIds(userIdList).then((userIdMap) => {
            result.map((commentVo) => {
              commentVo.commentatorName = userIdMap.get(commentVo.commentatorId)?.name;
              ...
              commentVo.childrenComment.map((childCommentVo) => {
                childCommentVo.commentatorName = userIdMap.get(childCommentVo.commentatorId)?.name;
                childCommentVo.commentatorAvatarImageLink = userIdMap.get(childCommentVo.commentatorId)?.avatarImageLink;
                childCommentVo.parentCommentatorName = userIdMap.get(childCommentVo.parentCommentatorId)?.name;
              });
            });
          });
        });
  ...
  async replyCommentByPostId(request: CommentRequest): Promise<CommentEntity> {
    ...
    commentEntity.rootCommentId = request.parentCommentId && await this.entityManager.createQueryBuilder().from(CommentEntity, "comment").where({
      postId: request.postId,
      postType: request.postType,
      validStatus: CommonValidStatus.VALID,
